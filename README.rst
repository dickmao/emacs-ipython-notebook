...
As an emacs user, you are likely to self-diagnose most simple problems.  First issue ``M-x ein:dev-start-debug``.  Then reproduce the error.  Higher level diagnostics appear in ``M-x ein:log-pop-to-all-buffer``.  Lower level diagnostics appear in ``M-x ein:log-pop-to-request-buffer``.
